...
// } catch (error) {
//   credentials = {
//     user: 'chill@triggerpointmedia.com',
//     password: 'admin',
//     network: 'Spectrio-Rush',
...
const numNetworks = credentialsSet.length;
var networkIndex = 0;

var retrieveNetworkPresentations = (networkIndex) => {

  const credentials = credentialsSet[networkIndex];
  ...
  bsnGetSession().activate(credentials.user, credentials.password, credentials.network, credentials.serverUrl)
    ...
    .then((items) => {
      ...
      bsnGetSession().deactivate();

      console.log('network presentations complete');

      networkIndex++;
      if (networkIndex < numNetworks) {
        console.log('move on to next network');
        retrieveNetworkPresentations(networkIndex);
      }
      console.log('all networks complete');
    })
    .catch((error) => console.log('Error: ', error.message));
  };

  retrieveNetworkPresentations(0);
